...
enum { SLOT_ACTIVE_COUNT=1 }; //charlie added change SLOT_ACTIVE_COUNT from 5 to 1
...
void log_core::_acquire_buffer_space_for_each(insert_info* info, long recsize)
{
...
//charlie added end
		...
		if(old_count == SLOT_AVAILABLE) {
			...
			assert(info->vthis()->count > SLOT_AVAILABLE); //SLOT_AVAILABLE = 0
			...
			membar_producer();
			info->count = SLOT_FINISHED - old_count; //SLOT_FINISHED = -4
		}
		else {
